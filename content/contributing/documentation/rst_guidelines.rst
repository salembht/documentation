...
An absolute file path indicates target's location from the root directory. A relative file path uses
smart notations (such as ``../`` that redirects to the parent folder) to indicate the target's
...
Add custom anchors above headings
---------------------------------
...
Systematically add custom anchors above the headings of your document so they can be used to
reference a specific section later from any documentation file or Odoo using documentation links.
...
recommended to accompany an icon with a :ref:`contributing-rst-guilabel` as a
descriptor.
...
referencing a specific part of a document by using the target as an anchor. When users on the
...
.. important::
   ...
   markup, the target must be named as follows:
...
.. note::
   - Add custom anchors for all headings so they can be referenced from any documentation file or
     from Odoo using documentation links.
...
.. important::
   :ref:`contributing-rst-relative-links` for documentation page
   hyperlinks.
...
.. list-table::
   ...
   * - .. code-block:: text
          ...
          Download this :download:`module structure template <extras/my_module.zip>` to start
...
Code tabs are essentially :ref:`group tabs <contributing-rst-group-tabs>` that treat
the content as a :ref:`code block <contributing-rst-code-blocks>`. The `code-tab`
markup is used to define a code tab. Just as for the `code-block` markup, the language defines the
