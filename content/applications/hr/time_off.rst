...
Odoo's **Time Off** app serves as a centralized hub for all time-off-related information. This
...
.. note::
   ...
   All users can access the *My Time Off*  (:menuselection:`Time Off app --> My Time --> My Time
   Off`) and *Overview* (:menuselection:`Time Off app --> Overview`) sections of the **Time Off**
   app. All other sections require specific access rights.
...
The **Time Off** app comes with four pre-configured time off types: :guilabel:`Paid Time Off`,
...
.. important::
   ...
   *accrual plan* to :guilabel:`At the start of the year`, it allows unused vacation time to
...
- :guilabel:`Company`: If in a multi-company database, the current company populates this field by
  ...
  .. note::
     ...
     :icon:`oi-settings-adjust` :guilabel:`optional columns` icon in the top-right corner of the
...
- :guilabel:`Departments`: This column is hidden by default. First, click the
  :icon:`oi-settings-adjust` :guilabel:`optional columns` icon in the top-right corner, next to
...
Any report can be added to a spreadsheet, when in either the :icon:`fa-area-chart` :guilabel:`Graph`
or :icon:`oi-view-pivot` :guilabel:`Pivot` view, through the *Insert in Spreadsheet* button that
appears in the top-left of the report.
...
:icon:`oi-view-list` :guilabel:`List` (the default view), :icon:`fa-area-chart` :guilabel:`Graph`,
...
:icon:`fa-area-chart` :guilabel:`Graph` (the default view), :icon:`oi-view-list` :guilabel:`List`,
